    ...
    Context 'Project Management Commands' {
        Context 'init-project command' {
            Context 'When initializing a new project' {
                BeforeAll {
                    # Arrange
                    $InitProjectPath = Join-Path $script:BaseProjectPath 'init-test'
                }
                
                It 'Should create proper project structure and settings.json' {
                    # Act
                    Write-Host "Executing: $script:ConsoleApp init-project --project $InitProjectPath" -ForegroundColor Cyan
                    $result = & $script:ConsoleApp init-project --project $InitProjectPath 2>&1
                    ...
                    Join-Path $InitProjectPath 'settings.json' | Should -Exist -Because 'settings.json should be created'

                    # Validate settings.json structure
                    $settingsPath = Join-Path $InitProjectPath 'settings.json'
            ...
            Context 'When project path already exists' {
                BeforeAll {
                    # Arrange
                    $ExistingProjectPath = Join-Path $script:BaseProjectPath 'existing-test'
                    New-Item -ItemType Directory -Path $ExistingProjectPath -Force | Out-Null
                }
                
                It 'Should handle existing directory gracefully' {
                    # Act
                    Write-Host "Executing: $script:ConsoleApp init-project --project $ExistingProjectPath" -ForegroundColor Cyan
                    $result = & $script:ConsoleApp init-project --project $ExistingProjectPath 2>&1
                    $exitCode = $LASTEXITCODE
    ...
    Context "Database Schema Operations" {
            ...
            Context "When extracting from valid database connection" {
                It "Should create semanticmodel.json with database schema" {
                    ...
                    if ($exitCode -eq 0) {
                        ...
                        $semanticModelPath | Should -Exist -Because "semanticmodel.json should be created"
        ...
        Context "data-dictionary command" {
            ...
            Context "When showing applied dictionaries" {
                BeforeAll {
                    # Arrange
                    $ShowDictDir = Join-Path $script:DbProjectPath "dict"
                    $ShowDictPath = Join-Path $ShowDictDir "show-test-dictionary.json"
                    $sampleDict = @{
                        objectType = "table"
                        schemaName = "dbo"
                        objectName = "Product"
                    ...
                    $sampleDict | ConvertTo-Json -Depth 3 | Set-Content $ShowDictPath
                }
                
                It "Should display dictionary information with --show option" {
                    # Act
                    Write-Host "Executing: $script:ConsoleApp data-dictionary --project $script:DbProjectPath --sourcePathPattern $ShowDictPath --objectType table --show" -ForegroundColor Cyan
                    $result = & $script:ConsoleApp data-dictionary --project $script:DbProjectPath --sourcePathPattern "$ShowDictPath" --objectType table --show 2>&1
                    $exitCode = $LASTEXITCODE
...
# Configure and run tests if executed directly
