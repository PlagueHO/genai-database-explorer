...
- OVR-003: Context: Operates within the model enrichment phase, using `IChatClientFactory` to parse markdown via a prompt template, project settings for type mapping/parallelism, and writes updates into the in-memory `SemanticModel`.

## 2. Architecture

- ARC-001: Design patterns
  - Dependency Injection for `IProject`, `IChatClientFactory`, `IPromptTemplateParser`, `ILiquidTemplateRenderer`, `ILogger`.
  - Factory pattern via `IChatClientFactory` to create configured AI clients.
...
- ARC-002: Dependencies
  ...
  - External: `Microsoft.Extensions.AI`, `Microsoft.Extensions.Logging`, `System.Text.Json`, `System.Resources`, filesystem.
  - Configuration: `IProject.Settings.DataDictionary.ColumnTypeMapping`, `IProject.Settings.SemanticModel.MaxDegreeOfParallelism`.
  - Prompts: `PromptTemplates/get_table_from_data_dictionary_markdown.prompt` (under application base directory at runtime).
- ARC-003: Interactions
  - Reads markdown files by glob/pattern; for each file, calls an `IChatClient` with a prompt template to produce a `TableDataDictionary`; merges into `SemanticModel` with type mapping and warnings.
...
flowchart TD
  subgraph DataDictionary
    ...
    PROM["PromptTemplate: get_table_from_data_dictionary_markdown.prompt"]
  end
...
classDiagram
  ...
  class DataDictionaryProvider {
    -_project: IProject
    -_chatClientFactory: IChatClientFactory
  ...
  class IChatClientFactory { <<external>> }
  class ILogger~DataDictionaryProvider~ { <<external>> }
  class ResourceManager { <<external>> }

  DataDictionaryProvider --> IChatClientFactory : uses
...
|-----------------|---------|------------|-------------|-------------|
| IDataDictionaryProvider.EnrichSemanticModelFromDataDictionaryAsync | Enrich semantic model from markdown dictionaries | semanticModel, sourcePathPattern, schemaName?, tableName? | Task | Uses IChatClient + prompt template; runs file parsing in parallel; logs warnings |
...
- IMP-001: Responsibilities
  ...
  - Markdown parsing: IChatClient with structured output (JSON schema) returns JSON that is deserialized to `TableDataDictionary`.
  - Merge behavior:
...
- IMP-002: Configuration and initialization
  - DI constructs `DataDictionaryProvider(IProject, IChatClientFactory, IPromptTemplateParser, ILiquidTemplateRenderer, ILogger<DataDictionaryProvider>)`.
  - Prompt template path resolved relative to `AppDomain.CurrentDomain.BaseDirectory` + `PromptTemplates` folder.
  - Settings used:
...
- QUA-001: Security: Reads local files; no direct DB access. AI calls use centralized `IChatClientFactory` for configured credentials/endpoints.
...
- REF-003: Testing guidelines
  - Mock `IChatClientFactory` and return a fake chat response for `TableDataDictionary`.
...
- REF-004: Troubleshooting
  ...
  - Empty AI response or deserialization errors: ensure prompt template file path resolves under the application base directory and the markdown format matches expectations.
