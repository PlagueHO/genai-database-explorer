# Technical Resources

This document contains links to technical documentation, repositories, and resources that are essential when working on the GenAI Database Explorer solution.

## Azure Cosmos DB

### Official Documentation

- [Azure Cosmos DB Documentation](https://learn.microsoft.com/en-us/azure/cosmos-db/)
- [Azure Cosmos DB for NoSQL](https://learn.microsoft.com/en-us/azure/cosmos-db/nosql/)
- [Azure Cosmos DB .NET SDK v3](https://learn.microsoft.com/en-us/azure/cosmos-db/nosql/sdk-dotnet-v3)

### Source Code & Samples

- [Azure Cosmos DB .NET v3 Repository](https://github.com/Azure/azure-cosmos-dotnet-v3) - Package source code
- [Azure Cosmos DB .NET v3 Samples](https://github.com/Azure/azure-cosmos-dotnet-v3/tree/master/Microsoft.Azure.Cosmos.Samples) - Code samples
- [Azure Cosmos DB Best Practices](https://learn.microsoft.com/en-us/azure/cosmos-db/nosql/best-practice-dotnet)

### Performance & Optimization

- [Performance tips for .NET SDK](https://learn.microsoft.com/en-us/azure/cosmos-db/nosql/performance-tips-dotnet-sdk-v3)
- [Partitioning in Azure Cosmos DB](https://learn.microsoft.com/en-us/azure/cosmos-db/partitioning-overview)
- [Request Units in Azure Cosmos DB](https://learn.microsoft.com/en-us/azure/cosmos-db/request-units)

## Azure OpenAI & Semantic Kernel

### Azure OpenAI

- [Azure OpenAI Service Documentation](https://learn.microsoft.com/en-us/azure/ai-services/openai/)
- [Azure OpenAI .NET SDK](https://learn.microsoft.com/en-us/dotnet/api/overview/azure/ai.openai-readme)
- [Azure OpenAI Quotas and Limits](https://learn.microsoft.com/en-us/azure/ai-services/openai/quotas-limits)

### Semantic Kernel

- [Semantic Kernel Documentation](https://learn.microsoft.com/en-us/semantic-kernel/)
- [Semantic Kernel GitHub Repository](https://github.com/microsoft/semantic-kernel)
- [Semantic Kernel .NET Samples](https://github.com/microsoft/semantic-kernel/tree/main/dotnet/samples)
- [Prompty Documentation](https://prompty.ai/)

### Token Management

- [Managing tokens in Azure OpenAI](https://learn.microsoft.com/en-us/azure/ai-services/openai/how-to/manage-tokens)
- [Token counting and estimation](https://platform.openai.com/tokenizer)

## .NET and C#

### .NET 10

- [.NET 10 Documentation](https://learn.microsoft.com/en-us/dotnet/core/whats-new/dotnet-10)
- [C# 14 Language Reference](https://learn.microsoft.com/en-us/dotnet/csharp/)
- [.NET API Browser](https://learn.microsoft.com/en-us/dotnet/api/)

### Libraries Used

- [System.CommandLine](https://learn.microsoft.com/en-us/dotnet/standard/commandline/) - CLI framework
- [Microsoft.Extensions.DependencyInjection](https://learn.microsoft.com/en-us/dotnet/core/extensions/dependency-injection)
- [Microsoft.Extensions.Logging](https://learn.microsoft.com/en-us/dotnet/core/extensions/logging)
- [Microsoft.Extensions.Options](https://learn.microsoft.com/en-us/dotnet/core/extensions/options)

### Best Practices

- [.NET Coding Conventions](https://learn.microsoft.com/en-us/dotnet/csharp/fundamentals/coding-style/coding-conventions)
- [Asynchronous Programming Patterns](https://learn.microsoft.com/en-us/dotnet/csharp/asynchronous-programming/)
- [Dependency Injection Best Practices](https://learn.microsoft.com/en-us/dotnet/core/extensions/dependency-injection-guidelines)

## Azure Infrastructure & Bicep

### Bicep

- [Bicep Documentation](https://learn.microsoft.com/en-us/azure/azure-resource-manager/bicep/)
- [Bicep GitHub Repository](https://github.com/Azure/bicep)
- [Azure Verified Modules (AVM)](https://azure.github.io/Azure-Verified-Modules/)
- [AVM Bicep Modules](https://github.com/Azure/bicep-registry-modules)

### Infrastructure as Code

- [Azure Resource Manager Templates](https://learn.microsoft.com/en-us/azure/azure-resource-manager/templates/)
- [Bicep Best Practices](https://learn.microsoft.com/en-us/azure/azure-resource-manager/bicep/best-practices)

## Azure Storage

### Azure Blob Storage

- [Azure Blob Storage Documentation](https://learn.microsoft.com/en-us/azure/storage/blobs/)
- [Azure Blob Storage .NET SDK](https://learn.microsoft.com/en-us/dotnet/api/overview/azure/storage.blobs-readme)
- [Blob Storage Performance Best Practices](https://learn.microsoft.com/en-us/azure/storage/blobs/storage-performance-checklist)

## Testing

### Unit Testing

- [MSTest Documentation](https://learn.microsoft.com/en-us/dotnet/core/testing/unit-testing-with-mstest)
- [Moq Documentation](https://github.com/devlooped/moq)
- [FluentAssertions Documentation](https://fluentassertions.com/introduction)

### Integration Testing

- [Integration Tests in .NET](https://learn.microsoft.com/en-us/aspnet/core/test/integration-tests)
- [Testing with .NET CLI](https://learn.microsoft.com/en-us/dotnet/core/tools/dotnet-test)

## Database Providers

### SQL Server

- [SQL Server Documentation](https://learn.microsoft.com/en-us/sql/)
- [Microsoft.Data.SqlClient](https://learn.microsoft.com/en-us/sql/connect/ado-net/microsoft-ado-net-sql-server)
- [SQL Server Best Practices](https://learn.microsoft.com/en-us/sql/relational-databases/performance/performance-center-for-sql-server-database-engine-and-azure-sql-database)

### ADO.NET

- [ADO.NET Overview](https://learn.microsoft.com/en-us/dotnet/framework/data/adonet/)
- [Connection Strings](https://learn.microsoft.com/en-us/dotnet/framework/data/adonet/connection-strings)

## Security & Authentication

### Managed Identity

- [Managed Identities Documentation](https://learn.microsoft.com/en-us/entra/identity/managed-identities-azure-resources/)
- [Azure Identity .NET SDK](https://learn.microsoft.com/en-us/dotnet/api/overview/azure/identity-readme)

### Security Best Practices

- [Azure Security Best Practices](https://learn.microsoft.com/en-us/azure/security/fundamentals/best-practices-and-patterns)
- [Secure Coding Guidelines](https://learn.microsoft.com/en-us/dotnet/standard/security/secure-coding-guidelines)

## CI/CD & DevOps

### GitHub Actions

- [GitHub Actions Documentation](https://docs.github.com/en/actions)
- [GitHub Actions for Azure](https://learn.microsoft.com/en-us/azure/developer/github/github-actions)
- [Azure CLI Reference](https://learn.microsoft.com/en-us/cli/azure/)

## Monitoring & Observability

### Application Insights

- [Application Insights Documentation](https://learn.microsoft.com/en-us/azure/azure-monitor/app/app-insights-overview)
- [OpenTelemetry in .NET](https://learn.microsoft.com/en-us/dotnet/core/diagnostics/observability-with-otel)

### Logging

- [Logging in .NET](https://learn.microsoft.com/en-us/dotnet/core/extensions/logging)
- [Structured Logging Best Practices](https://learn.microsoft.com/en-us/dotnet/core/extensions/logging-guidance)

## Additional Resources

### Design Patterns

- [Enterprise Application Patterns](https://learn.microsoft.com/en-us/azure/architecture/patterns/)
- [Repository Pattern](https://learn.microsoft.com/en-us/dotnet/architecture/microservices/microservice-ddd-cqrs-patterns/infrastructure-persistence-layer-design)

### PowerShell

- [PowerShell Documentation](https://learn.microsoft.com/en-us/powershell/)
- [Pester Testing Framework](https://pester.dev/)

## Community & Support

- [.NET Community](https://dotnet.microsoft.com/platform/community)
- [Azure Community](https://azure.microsoft.com/en-us/community/)
- [Stack Overflow - Azure Cosmos DB](https://stackoverflow.com/questions/tagged/azure-cosmosdb)
- [Stack Overflow - Semantic Kernel](https://stackoverflow.com/questions/tagged/semantic-kernel)
