...
Describe 'GenAI Database Explorer Console Application - LocalDisk Strategy' {
    BeforeAll {
        ...
        $testConfig = Initialize-TestEnvironment -PersistenceStrategy 'LocalDisk' -TestFilter $TestFilter
        ...
        Test-RequiredEnvironmentVariables -Environment $script:TestEnv -NoAzureMode $script:NoAzureMode -PersistenceStrategy 'LocalDisk'

        $consoleAppPath = Resolve-ConsoleAppPath

        $workspaceConfig = Initialize-TestWorkspace -TestDriveRoot $TestDrive -ConsoleAppPath $consoleAppPath -TempDirPrefix 'genaidb-localdisk-test'
