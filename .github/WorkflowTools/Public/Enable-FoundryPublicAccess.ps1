<#
    .SYNOPSIS
    Enables public network access on a Microsoft Foundry (Cognitive Services) account and waits until the resource is ready.

    .DESCRIPTION
    Resolves the Microsoft Foundry account name from an explicit parameter or from a SQL server naming pattern, waits for the account to reach a terminal provisioning state with a valid endpoint,
...
function Enable-FoundryPublicAccess {
    ...
    begin {
        ...
        Write-Verbose "Starting Microsoft Foundry public access enablement"
        ...
        function Wait-ForFoundryReady {
            ...
            throw "Microsoft Foundry service '$Name' was not ready within timeout ($TimeoutSeconds s)."
        }
    ...
    process {
        try {
            ...
            $resolvedFoundryName = if (-not [string]::IsNullOrEmpty($FoundryName)) {
                ...
                Write-Host "Constructed Microsoft Foundry service name from SQL server: $computedName"
                $computedName
            } else {
                Write-Warning "Could not determine Microsoft Foundry service name (no input and SQL name not matching pattern). Skipping public access configuration."
                return
            }

            Write-Host "Waiting for Microsoft Foundry service '$resolvedFoundryName' to be ready..."
            ...
            $target = "Microsoft Foundry '$resolvedFoundryName' in resource group '$ResourceGroupName'"
            ...
            if ($updatedPublicNetworkAccess -eq "Enabled" -and $updatedDefaultAction -eq "Allow") {
                Write-Host "✅ Public network access enabled for Microsoft Foundry service" -ForegroundColor Green
            } else {
            ...
            Write-Error "Failed to enable public access for Microsoft Foundry: $($_.Exception.Message)"
            throw
        }
    }
    
    end {
        Write-Verbose "Microsoft Foundry public access enablement completed"
    }
}