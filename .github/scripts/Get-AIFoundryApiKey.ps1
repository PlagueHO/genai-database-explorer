function Get-AIFoundryApiKey {
    ...
    .EXAMPLE
    Get-AIFoundryApiKey -ResourceGroupName 'rg-test' -AIFoundryName 'aif-123'

    .EXAMPLE
    Get-AIFoundryApiKey -ResourceGroupName 'rg-test' -SqlServerName 'sql-123'
    
    .OUTPUTS
    None. Exports AZURE_OPENAI_API_KEY to $env:GITHUB_ENV.
    
    .NOTES
    This function retries with exponential backoff when key retrieval fails.
    #>
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)]
    ...
    begin {
        ...
        Write-Verbose "Starting Get-AIFoundryApiKey process"
    }
    ...
    process {
        ...
        Write-Verbose "Get-AIFoundryApiKey process completed"
    }
}

# Call the function with script parameters when run as script
if ($MyInvocation.InvocationName -ne '.') {
    Get-AIFoundryApiKey @PSBoundParameters
}
