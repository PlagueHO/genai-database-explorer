...
.EXAMPLE
./Remove-SqlServerFirewallRule.ps1 -ResourceGroupName 'rg-test' -SqlServerName 'sql-test' -FirewallRuleName 'GitHubRunner-123'

.EXAMPLE
./Remove-SqlServerFirewallRule.ps1 -ResourceGroupName 'rg-test' -SqlServerName 'sql-test' -FirewallRuleName 'GitHubRunner-123' -IgnoreNotFound

.OUTPUTS
None. Removes the specified SQL Server firewall rule.

.NOTES
This script is designed for cleanup operations in CI/CD scenarios and requires Azure PowerShell modules.
...
Write-Verbose "SQL Server firewall rule removal completed"