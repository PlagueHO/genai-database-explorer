function Test-IntegrationTestResults {
    ...
    .EXAMPLE
    Test-IntegrationTestResults -TestResultsPath './custom-results/tests.xml' -OutputVariable 'custom-results-valid'

    .EXAMPLE
    Test-IntegrationTestResults -ShowPreview -PreviewLength 1000

    .OUTPUTS
    None. Sets GitHub Actions output variables and displays validation results.

    .NOTES
    This function is designed for GitHub Actions workflows and requires the GITHUB_OUTPUT environment variable.
    ...
    begin {
        ...
        Write-Verbose "Starting Test-IntegrationTestResults process"
    ...
    process {
        ...
        Write-Verbose "Test-IntegrationTestResults process completed"
    }
}

# Call the function with script parameters when run as script
if ($MyInvocation.InvocationName -ne '.') {
    Test-IntegrationTestResults @PSBoundParameters
}